...
class PlayerMusica(ctk.CTkToplevel):
    ...
    def __init__(self, master, usuario_logado):
        ...
        logging.info(f"Player iniciado para o usuário: {self.usuario_logado['nome']}") # Log inicial
    ...
    def salvar_dados_usuario(self):
        ...
        if os.path.exists(arquivo_usuarios):
            ...
            logging.info(f"Dados do usuário '{self.usuario_logado['nome']}' salvos com sucesso.")
            with open(arquivo_usuarios, 'w', encoding='utf-8') as file:
                yaml.dump(usuarios, file, allow_unicode=True)
                
    def setup_logging(self):
        try:
            log_file_path = 'changes.log'
            ...
            print(f"Logging configurado. O arquivo de log será salvo em: {log_file_path}")

        except Exception as e:
            print(f"ERRO CRÍTICO: Não foi possível configurar o logging. Erro: {e}")
    ...
    def setup_biblioteca(self, parent):
        ...
        ctk.CTkButton(frame_biblioteca, text="+ Adicionar a Playlist", 
                     command=self.adicionar_a_playlist).pack(fill="x", padx=5, pady=5)
    ...
    def adicionar_musica(self):
        ...
        if arquivos:
            logging.info(f"{len(arquivos)} música(s) adicionada(s) à biblioteca.")
    ...
    def selecionar_musica(self, caminho):
        ...
        logging.info(f"Música selecionada na biblioteca: '{caminho}'")
    ...
    def tocar_da_biblioteca(self):
        ...
        if hasattr(self, 'musica_selecionada'):
            ...
            logging.warning("Tentativa de tocar da biblioteca sem música selecionada.")
    ...
    def tocar_musica(self, caminho):
        ...
        try:
            ...
            logging.info(f"Iniciando reprodução de: '{nome_musica}'")
            ...
            logging.error(f"Erro ao tentar tocar a música '{caminho}': {e}")
    ...
    def criar_playlist(self):
        ...
        if nome:
            ...
            self.salvar_dados_usuario()
            self.atualizar_lista_playlists()
            logging.info(f"Playlist '{nome}' criada para o usuário '{self.usuario_logado['nome']}'.")
    ...
    def adicionar_a_playlist(self):
        ...
        if not self.playlist_atual:
            logging.warning("Tentativa de adicionar música sem playlist selecionada.")
            messagebox.showwarning("Aviso", "Selecione uma playlist primeiro!")
            return
        
        if not hasattr(self, 'musica_selecionada'):
            
            logging.warning("Tentativa de adicionar música sem música selecionada.")            
            messagebox.showwarning("Aviso", "Selecione uma música primeiro!")
            return
    ...
    def deletar_playlist(self):
        ...
        if not self.playlist_atual:
            logging.warning("Tentativa de deletar sem playlist selecionada.")
            messagebox.showwarning("Aviso", "Selecione uma playlist primeiro!")
            return
        ...
        if resposta:
            ...
            logging.info(f"Playlist deletada com sucesso.")
    ...
    def sair(self):
        """Sai do player"""
        logging.info(f"Player fechado para o usuário: {self.usuario_logado['nome']}")
        pygame.mixer.music.stop()
        self.destroy()
